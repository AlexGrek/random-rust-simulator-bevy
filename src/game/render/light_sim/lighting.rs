...
fn setup_directional_lights(app: &mut App) {
    ...
    app.add_systems(PostUpdate, simulation::run_fucking_simulation);
}

#[derive(Resource)]
pub struct LightOverlayTextureHandle(pub Handle<Image>);

fn setup_example(mut commands: Commands, mut images: ResMut<Assets<Image>>) {
    let IMAGE_WIDTH = 16;
    let IMAGE_HEIGHT = 16;
...
fn setup_overlay(
    ...
    let handle = images.add(image);
    // Additive Blend
    let additive_material = ColorMaterial {
